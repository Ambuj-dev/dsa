...
public class MaximumRibbonCountTabulation {
    ...
    public int countRibbonPiecesOptimised(int[] ribbonLengths, int total)
    {
        int n = ribbonLengths.length;
        ...
        for(int i=0; i < n; i++) {
            for(int t=1; t <= total; t++) {
                ...
                // include the ribbon and check if the remaining length can be cut into available lengths
                if(t >= ribbonLengths[i] && dp[t-ribbonLengths[i]] != Integer.MIN_VALUE)
                    dp[t] = Math.max(dp[t], dp[t-ribbonLengths[i]]+1);
            }
        }
    ...
    public static void main(String[] args) {
        MaximumRibbonCountTabulation cr = new MaximumRibbonCountTabulation();
        int[] ribbonLengths = {2,3,5};
        System.out.println(cr.countRibbonPieces(ribbonLengths, 5));
        System.out.println(cr.countRibbonPiecesOptimised(ribbonLengths, 5));
        ribbonLengths = new int[]{2,3};
        System.out.println(cr.countRibbonPieces(ribbonLengths, 7));
        System.out.println(cr.countRibbonPiecesOptimised(ribbonLengths, 7));
        ribbonLengths = new int[]{5,3,7};
        System.out.println(cr.countRibbonPieces(ribbonLengths, 13));
        System.out.println(cr.countRibbonPiecesOptimised(ribbonLengths, 13));
