...
public class RodCutting {
    ...
    public int solveRodCuttingOptimised(int[] lengths, int[] prices, int n) {
        // base checks
        if (n <= 0 || prices.length == 0 || prices.length != lengths.length)
            return 0;
        ...
        for(int i=0; i < lengthCount; i++) {
            for(int len=1; len <= n; len++) {
                int p1=0, p2=0;
                if(lengths[i] <= len)
                    p1 = prices[i] + dp[len-lengths[i]];
                if( i > 0 )
                    p2 = dp[len];
                dp[len] = Math.max(p1, p2);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        maxProfit = rc.solveRodCuttingOptimised(lengths, prices, 5);
        System.out.println(maxProfit);
    }
}
